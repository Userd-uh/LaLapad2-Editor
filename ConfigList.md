...
モード変更は Save All 後に左右のファームウェア再ビルド・書き込みが必要です。
Actionの割り当ては、76位置対応ファームウェアへ更新した後は既存のStudio RPCで反映できます。
2F/3Fは左右・方向・レイヤーごとに独立しています。旧68位置は維持し、3F用に68〜75を追加しました。
更新前にはMCUのキーマップを読み込み、Export keymapで退避してください。
旧ファームウェアに対する76位置のRPC書き込みは、変更送信前に拒否します。
3F Scrollは直接のホイール出力であり、2Fの平滑化・慣性処理とは独立です。
